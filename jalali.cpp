...
int jalali_jd0_cpp(int year)
{
    ...
    for( i = 0; i < 12; i++)
        if( year < breaks[i])
        {
            rval = JALALI_ZERO + year * 365 +
                ( deltas[i] + year * 303) / 1250;
...
void get_jalali_year_data(int year, int *days, int *month_data)
{
   days[0] = jalali_jd0_cpp( year) + 1;
   days[1] = jalali_jd0_cpp( year + 1) + 1;
...
int get_calendar_data(int year, int *days, int *month_data)
{
   int rval = 0;

   memset( month_data, 0, N_MONTHS);
   get_jalali_year_data( year, days, month_data);
...
int dmy_to_day(int day, int month, int year)
{
   int mdata[N_MONTHS];
   int jd;
   int year_ends[2];
...
void day_to_dmy(int jd, int *day, int *month, int *year)
{
   int year_ends[2];
   int curr_jd;
   int month_data[N_MONTHS];
